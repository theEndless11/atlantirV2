            ...
            return __generator(this, function (_g) {
                switch (_g.label) {
                    case 0:
                        ...
                        nangoApp = this._appForAction(args.app) // ← this line must be here
                        ;
                        connectionId = workspaceId + "-" + employeeId + "-" + nangoApp // ← nangoApp not args.app
                        ;
                    ...
                    case 2:
                        body = _g.sent();
                        clean = body.startsWith('<') ? "HTTP " + sessionRes.status + " from " + nangoHost() + "/connect/sessions \u2014 check NANGO_HOST points to port 8080" : body;
                    ...
                    case 4:
                        ...
                        if (rawConnectLink) {
                            try {
                                ...
                                base = nangoConnectUiUrl() // e.g. https://nango.sonarbay.com/connect
                                ;
            ...
            return __generator(this, function (_d) {
                switch (_d.label) {
                    case 0:
                        ...
                        nangoApp = this._appForAction(app) // ← add this
                        ;
                        connectionId = workspaceId + "-" + employeeId + "-" + nangoApp // ← use nangoApp
                        ;
                        return [4 /*yield*/, fetch(nangoHost() + "/connection/" + connectionId + "?provider_config_key=" + nangoApp, // ← nangoApp
                            { headers: nangoHeaders() })];
                    case 1:
                        res = _d.sent();
                        if (res.status === 404)
            ...
            return __generator(this, function (_e) {
                switch (_e.label) {
                    case 0:
                        ...
                        if (!approvalTokenId) {
                            throw new types_1.ToolExecutorError('APPROVAL_REQUIRED', "Action \"" + action + "\" requires an approval token. Obtain approval before executing.");
                        }
                        return [4 /*yield*/, sb
                    ...
                    case 12:
                        ...
                        return [4 /*yield*/, sb.from('tool_audit_log').update({
                                result_code: 'error',
                                latency_ms: latencyMs,
                                error: err_1.message
                            }).eq('id', auditId)];
