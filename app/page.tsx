...
export default function LandingPage() {
  ...
  useEffect(() => {
    ...
    sb.auth.getSession().then(async ({ data: { session } }) => {
      ...
      else { router.replace('/onboarding'); setChecking(false) }
    })
  ...
  if (checking) return (
    <div style={{ position:'fixed', inset:0, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', background:'#020d07', zIndex:9999 }}>
      <svg width="36" height="36" viewBox="0 0 32 32" fill="none" style={{ marginBottom:20 }}>
        <path d="M16 3L28 9.5V22.5L16 29L4 22.5V9.5L16 3Z" fill="rgba(52,211,153,0.15)" stroke="rgba(52,211,153,0.55)" strokeWidth="1.2"/>
      ...
      <div style={{ display:'flex', gap:6 }}>
        {[0,140,280].map(d=><span key={d} style={{ width:6,height:6,borderRadius:'50%',background:'#34d399',opacity:0.4,animation:`lp-pulse 1.2s ${d}ms ease-in-out infinite`,display:'inline-block' }}/>)}
      </div>
      <style>{`@keyframes lp-pulse{0%,80%,100%{transform:scale(1);opacity:.35}40%{transform:scale(1.4);opacity:1}}`}</style>
    </div>
  )
